...
# to decompose absorption secptra into two populations.
# <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
...
absorption <- absorption %>%
  ...
  mutate(mod_s = map(
    data,
    ~ minpack.lm::nlsLM(
      phytoplankton_absorption ~ (as1 - as2) / s * (1. - exp(-s * hplc_chla)) + as2 *
        .$hplc_chla,
      data = .,
...
absorption <- absorption %>%
  ...
  mutate(mod_as1_as2 = map2(
    ...
    average_s,
    ~ minpack.lm::nlsLM(
      phytoplankton_absorption ~ (as1 - as2) / ..2 * (1. - exp(-..2 * hplc_chla)) + as2 *
        hplc_chla,
      data = ..1,
