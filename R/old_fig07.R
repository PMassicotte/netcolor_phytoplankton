...
res <- df %>%
  ...
  mutate(snap_aphy_ccf_mod = map(data, ~ ccf(
    .$anap, .$aphy_specific,
    plot = FALSE, lag.max = 30
  ))) %>%
  mutate(snap_aphy_ccf_tidy = map(snap_aphy_ccf_mod, broom::tidy)) %>%
  unnest(snap_aphy_ccf_tidy)
...
p2 <- df %>%
  ...
  summarise(across(c(anap, aphy_specific), mean)) %>%
  ggplot(aes(x = anap, y = aphy_specific)) +
