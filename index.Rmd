...
-   Figure 1 : la carte des stations (il y a certains points qui devront bouger).
...
Temporal autocorrelation between `aphy*` and `anap` at 443 nm.

There are quite many gaps in the data. It was therefore impossible to go lag-correlation because one need one observation per day. To do it so, I took the LOESS models calculated in Fig. 5. Then, I randomly generated data (one observation per day) within the standard error intervals found in Fig. 5. *I repeated the process 1000 times.* After that, I calculated the auto-correlation with a max gap of +- 30 days for each of the simulation. Finally, I calculated the average correlation per lag over the 1000 simulation. The next graph shows the lag-correlation between `aphy*(443)` and `anap(443)`. One can see that there is a gap of ~12 days between the two variables in the Labrador data.
...
**(A)** Simulated examples of `aphy*(443)` and `anap(443)` at a one-day temporal resolution. The black and red lines show the LOESS model and the 99% confidence interval as they appear in Fig. 5. **(B)** Scatterplots showing the relationships between simulated `aphy*(443)` and `anap(443)` at lag 0.
